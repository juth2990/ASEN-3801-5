...
doublet_time = 0.25;
doublet_size = 15*(pi)/180; % radians

% run ode45 with EOM function
ode = @(t, x) AircraftEOMDoublet(t, x, aircraft_surfaces, doublet_size,doublet_time, wind_inertial, aircraft_parameters); 
options = odeset('RelTol', 1e-6, 'AbsTol', 1e-8); 
[t, x] = ode45(ode, tspan, aircraft_state, options); 
aircraft_surfaces = aircraft_surfaces.*ones(4,length(t));
for i = 1:length(t)
     if 0 < t(i) && t(i) <= doublet_time
        deltaE = aircraft_surfaces(1,i) + doublet_size;
    elseif doublet_time < t(i) && t(i) <= 2*doublet_time
        deltaE = aircraft_surfaces(1,i) - doublet_size;
    else
        deltaE = aircraft_surfaces(1,i);
     end
     aircraft_surfaces(1,i)=deltaE;
end
PlotAircraftSim(t, x', aircraft_surfaces,fig, col,'3.1')
end
...
PlotAircraftSim(t, x', aircraft_surfaces,fig, col,'3.2')
end
